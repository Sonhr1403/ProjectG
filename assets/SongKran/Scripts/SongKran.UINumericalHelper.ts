...
@ccclass
export default class SongKranNumericalHelper extends cc.Component {
    public static instance: SongKranNumericalHelper = null;
    public static scheduleForLabel(label: cc.Label, data: number, duration: number = 0.5, proto: string = "%s") {
        let comp = label.node.getComponent(SongKranNumericalHelper);
        if (!comp) {
            comp = label.node.addComponent(SongKranNumericalHelper);
    ...
    onLoad() {
        SongKranNumericalHelper.instance = this;
        if (!this.label) {

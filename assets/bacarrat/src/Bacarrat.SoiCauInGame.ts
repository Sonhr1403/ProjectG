...
import { LobbyConst } from "../../lobby/LobbyConst";
import { Bacarrat_Const } from "./Bacarrat.Const";

const { ccclass, property } = cc._decorator;

@ccclass
export default class Bacarrat_SoiCauInGame extends cc.Component {
    ...
    updateCau() {
        ...
        for (let i = 0; i < this.listCau.childrenCount; i++) {
            ...
            if (startPos + i < this._data.length) {
                nodeCau.active = true;
                let type = this._data[startPos + i].listWinType;
                this.setCauInfo(type, nodeCau);
            } else {
    ...
    setCauInfo(type, nodeCau) {
        ...
        for (let i = 0; i < type.length; i++) {
            nodeCau.getChildByName('lb_score').active = false

            if (type[i] <= Bacarrat_Const.TAG_POT.TIE) {
                nodeCau.getComponent(cc.Sprite).spriteFrame = this.iconScore[type[i]];
            }
            if (type[i] === Bacarrat_Const.TAG_POT.SUPER_6) {
                nodeCau.getChildByName('lb_score').active = true
            }
        }
    }
}
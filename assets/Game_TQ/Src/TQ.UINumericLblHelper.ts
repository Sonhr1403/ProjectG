...
@ccclass
export default class TQNumericalHelper extends cc.Component {
    public static instance: TQNumericalHelper = null;
    public static scheduleForLabel(label: cc.Label, data: number, duration: number = 0.5, proto: string = "%s") {
        let comp = label.node.getComponent(TQNumericalHelper);
        if (!comp) {
            comp = label.node.addComponent(TQNumericalHelper);
    ...
    onLoad() {
        TQNumericalHelper.instance = this;
        if (!this.label) {

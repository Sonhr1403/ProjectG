...
@ccclass
export default class ThanhGiongNumericalHelper extends cc.Component {
    public static instance: ThanhGiongNumericalHelper = null;
    public static scheduleForLabel(label: cc.Label, data: number, duration: number = 0.5, proto: string = "%s") {
        let comp = label.node.getComponent(ThanhGiongNumericalHelper);
        if (!comp) {
            comp = label.node.addComponent(ThanhGiongNumericalHelper);
    ...
    onLoad() {
        ThanhGiongNumericalHelper.instance = this;
        if (!this.label) {

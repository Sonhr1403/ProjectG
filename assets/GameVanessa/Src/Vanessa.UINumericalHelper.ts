...
@ccclass
export default class VanessaNumericalHelper extends cc.Component {
    public static instance: VanessaNumericalHelper = null;
    public static scheduleForLabel(label: cc.Label, data: number, duration: number = 0.5, proto: string = "%s") {
        let comp = label.node.getComponent(VanessaNumericalHelper);
        if (!comp) {
            comp = label.node.addComponent(VanessaNumericalHelper);
    ...
    onLoad() {
        VanessaNumericalHelper.instance = this;
        if (!this.label) {

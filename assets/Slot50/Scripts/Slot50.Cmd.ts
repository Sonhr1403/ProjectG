...
export namespace Slot50Cmd {
  ...
  export interface ImpColumn { list: Array<Slot50Cmd.ImpItemCell>, itemFrame: ImpFrame, }
  ...
  export class Send {
    public static sendSlotExitGame(roomId: number) {
      let pk = new Slot50Cmd.SlotSendExitGame();
      pk.roomId = roomId;
      Slot50Connector.instance.sendPacket(pk);
    }

    public static sendSlotJoinGame(betAmount = 0) {
      console.log("GỬI LÊN", betAmount);
      let pk = new Slot50Cmd.SlotSendJoinGame();
    ...
    public static sendSlotBet(betAmount: number) {
      ...
      let pk = new Slot50Cmd.SlotSendBet();
      pk.betAmount = betAmount;
      Slot50Connector.instance.sendPacket(pk);
    }

    public static sendOpenJackpot(index: number) {
      let pk = new Slot50Cmd.SlotSendOpenJackpot();
      pk.index = index;
      Slot50Connector.instance.sendPacket(pk);
    }

    public static sendSelectFreeGame(option: number) {
      let pk = new Slot50Cmd.SlotSendSelectFreeGame();
      pk.option = option;
      Slot50Connector.instance.sendPacket(pk);
    }

    public static sendStartFreeGame() {
      let pk = new Slot50Cmd.SlotSendStartFreeGame();
      Slot50Connector.instance.sendPacket(pk);
    }

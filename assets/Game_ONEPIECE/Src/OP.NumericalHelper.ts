...
@ccclass
export default class OPNumericalHelper extends cc.Component {
    public static instance: OPNumericalHelper = null;
    public static scheduleForLabel(label: cc.Label, data: number, duration: number = 0.5, proto: string = "%s") {
        let comp = label.node.getComponent(OPNumericalHelper);
        if (!comp) {
            comp = label.node.addComponent(OPNumericalHelper);
    ...
    onLoad() {
        OPNumericalHelper.instance = this;
        if (!this.label) {

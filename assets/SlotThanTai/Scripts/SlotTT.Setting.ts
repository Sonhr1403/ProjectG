...
@ccclass
export default class SlotTTSettings extends cc.Component {
  public static instance: SlotTTSettings = null;
  ...
  @property(cc.Button)
  private languageToggler: cc.Button = null; // 0H -> 222H // 113: vietnamese

  @property(cc.Node)
  private maskLanguageOption: cc.Node = null; // 0H -> 222H // 113: vietnamese
  ...
  private sliderVolumeCallback(slider: cc.Slider, customEventData: string) {
    ...
    SlotTTMusicManager.instance.setSystemVolume(this.volumeSlider.progress);
  ...
  private toggleLanguageOptions() {
    ...
    if (this.languageOptionEnabled == false) {
      ...
      cc.tween(this.maskLanguageOption).to(0.2, { height: 113 }).start();
    } else {
  ...
  private toggleMusic() {
    ...
    this.updateMusic();
    SlotTTMusicManager.instance.setMusicStatus(this.musicEnabled);
  }
  ...
  private updateMusic() {
    if (this.musicEnabled === false) {
      ...
      SlotTTMusicManager.instance.setSystemVolume(0);
      // SlotTTMusicManager.instance.stopAll();
      SlotTTMusicManager.instance.pauseSlotMusic();
    } else {
      ...
      SlotTTMusicManager.instance.setSystemVolume(this.volumeSlider.progress);
      SlotTTMusicManager.instance.resumeSlotMusic();
    }
  }
